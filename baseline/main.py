...
def calculate_metrics(eval_tracker: Dict) -> Dict:
    ...
    # Count successes - handle None results
    task_goal_correct = sum(
        1 for task in eval_tracker.values() 
        if task.get("result") is not None and task.get("result", {}).get("correct", False)
    )
    
    subgoal_correct = sum(
        task.get("result", {}).get("subgoal_correct", 0) if task.get("result") is not None else 0
        for task in eval_tracker.values()
    )
    
    total_subgoals = sum(
        task.get("result", {}).get("total_subgoals", 0) if task.get("result") is not None else 0
        for task in eval_tracker.values()
    )
    ...
    tgc = (task_goal_correct / total_tasks) * 100 if total_tasks > 0 else 0.0
